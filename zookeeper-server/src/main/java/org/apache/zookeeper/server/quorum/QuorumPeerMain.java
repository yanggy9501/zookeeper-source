 ...
 * 服务端启动类
 * 作用：
 * 1. 加载zoo.cfg配置，存放到配置实例QuorumPeerConfig中
 * 2. 开启定时清理事务日志与快照数据文件的任务，DataDirCleanupManager
 * 3. 根据zoo.cfg文件中是否配置了server.*属性判断是集群模式还是单点模式
 */
...
public class QuorumPeerMain {
    ...
    /**
     * zookeeper server 启动类，需要配置启动参数指定 zookeeper 配置文件 如："zookeeper/conf/zoo3.cfg"
     * Quorum：仲裁
     *
    ...
    protected void initializeAndRun(String[] args) throws ConfigException, IOException, AdminServerException {
        ...
        if (args.length == 1) {
            // args[0] 启动参数 zoo.cfg 的文件路径，如 "E:/volume/java/resource code/zookeeper/zookeeper/conf/zoo.cfg"
            // 注意有空时要使用冒号
            config.parse(args[0]);
        }
    ...
    public void runFromConfig(QuorumPeerConfig config) throws IOException, AdminServerException {
        ...
        try {
            ...
            // 启动 + 选举，注意：QuorumPeer 也是一个线程
            // start 方法先做准备工作，然后启动线程 -> run()
            quorumPeer.start();
            ZKAuditProvider.addZKStartStopAuditLog();
            // 线程阻塞等待：主线程等待 quorumPeer 线程执行完毕
            quorumPeer.join();
