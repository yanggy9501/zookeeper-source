    ...
    void followLeader() throws InterruptedException {
        ...
        try {
            ...
            try {
                ...
                // 连接 leader，完成 leader 到 follower 的数据同步，可能出现这样一种情况：a (leader) b (follower) c (follower) d (follower) e (follower)
                // a 宕机了，a 同步到所有节点的数据中 b 同步数据量最大，其他没有同步完成，b 被选举为 leader，在重新连接后，b 需要将其高出 c d e的那部分数据同步下去
